...
Deno.serve(async (req) => {
  ...
  try {
    await log('info', `Starting scrape for ${store.name}`);
    ...
    while (nextUrl) {
      ...
      await log('info', `Fetching page ${page} for ${store.name}`);
      ...
      for (const product of products) {
        ...
        // Check existing product for content hash
        ...
        if (hashChanged) {
          upsertData.last_changed_at = new Date().toISOString();
        }
